        ...
        Wrap(
          ...
          children: [
            ...
            for (final category in visibleCustoms)
              _categoryChip(category, category.name == selectedCategory, () => onSelected(category.name)),
            // Always rendered now — this was previously wrapped in
            // "if (hasMoreCustoms)", which meant it disappeared entirely
            // whenever there were zero (or few) custom categories, with
            // no other way to reach the "+ Add New" action inside the
            // dialog it opens. Now it's a permanent entry point.
