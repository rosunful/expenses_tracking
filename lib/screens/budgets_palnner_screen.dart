...
class _BudgetPlannerScreenState extends State<BudgetPlannerScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Live — reflects whatever's currently selected, not locked to
    // anything, since a budget is an ongoing target, not a one-time
    // historical record like a transaction.
...
class _BudgetCardState extends State<_BudgetCard> {
  ...
  Future<void> _save() async {
    ...
    // Drop focus BEFORE disabling the field — flipping enabled:false
    // on a still-focused TextField in the same frame as the keyboard
    // dismissing is what can trip Flutter's InputDecorator layout
    // assertion (debugNeedsLayout).
        ...
        child: Column(
          ...
          children: [
            DropdownButtonFormField<String>(
              // Key tied to the relevant state — forces Flutter to fully
              // recreate this widget (not just rebuild it) whenever
              // _selectedCategory changes from OUTSIDE the dropdown's own
              // onChanged (like right after adding a new category), so
              // its internal selection state can never silently desync
