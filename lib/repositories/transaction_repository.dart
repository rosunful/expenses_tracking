...
class TransactionRepository {
  ...
  /// The soft delete — sets isHidden true rather than removing the
  /// document. This is what the Activity screen's Delete button calls.
  Future<void> hideTransaction(String transactionId) async {
    await _transactionsRef.doc(transactionId).update({'isHidden': true});
  }

  /// Brings a hidden transaction back — for the future History screen's
  /// Undo action.
  Future<void> unhideTransaction(String transactionId) async {
    await _transactionsRef.doc(transactionId).update({'isHidden': false});
  }

  /// The real, permanent removal — kept as-is from before. Not called
  /// anywhere yet; this is what the future History screen's "Delete
  /// Permanently" will use, same pattern as Budget/Goal/Reminder history.
